# AGENTS.md — Royal Global Apparel

This document provides an overview of the project structure for AI agents and developers working on this codebase.

## Project Overview

A premium women's fashion brand website for Royal Global Apparel (Onitsha, Nigeria). Features a public landing page with products, about, testimonials carousel, and order form — plus a password-protected admin dashboard for product management. Built with TanStack Start and deployed on Netlify.
...
src/
  routes/
    __root.tsx          — Root layout with HTML shell and site-wide meta tags
    index.tsx           — Full landing page (hero, products, about, testimonials, order form, footer)
    admin/index.tsx     — Password-protected admin dashboard (add/delete products)
  server/
    products.functions.ts  — getProducts, addProduct, deleteProduct via Netlify Blobs
    admin.functions.ts     — verifyAdminPassword (checks ADMIN_PASSWORD env var)
public/
  logo.jpg             — RGA brand logo (circular green/white)
  order-form.html      — Static Netlify Forms skeleton for build-time detection
```

## Key Decisions

- **Admin auth**: `sessionStorage` flag set after `verifyAdminPassword` server function confirms password. Password defaults to `Miracle12345`, configurable via `ADMIN_PASSWORD` env var.
- **Product storage**: Netlify Blobs (`products` store, key `all`) as JSON array. Falls back to `STARTER_PRODUCTS` on first load.
- **Order form**: Netlify Forms — React form POSTs to `/order-form.html` with `form-name: order`. Email notifications configured in Netlify dashboard.
- **Testimonials**: 8 hard-coded testimonials from different Nigerian tribes, carousel auto-rotates every 5 seconds, shows 3 at once on desktop.

## Environment Variables

- `ADMIN_PASSWORD` — Admin password (default: `Miracle12345`)
